@echo off
chcp 65001 >nul
title 1학년 1반 교실 알림 설치
echo 1학년 1반 교실 알림 설치
echo.
...
:: 시작 프로그램 등록
set "STARTUP_BAT=%STARTUP%\ClassroomAlert_1-1.bat"
> "%STARTUP_BAT%" echo @echo off
>> "%STARTUP_BAT%" echo start "" /min powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File "%INSTALL_DIR%\ClassroomAlert.ps1" -CLS "1-1"

:: 즉시 실행
start "" /min powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File "%INSTALL_DIR%\ClassroomAlert.ps1" -CLS "1-1"
