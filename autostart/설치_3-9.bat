...
set "USERDATA=%LOCALAPPDATA%\ClassroomAlert\3-9"

taskkill /f /im wscript.exe >nul 2>&1
timeout /t 3 /nobreak >nul
del "%LOCALAPPDATA%\ClassroomAlert\3-9\watcher.lock" >nul 2>&1
...
>> "%VBS%" echo sUserData = sParent ^& "\3-9"
...
echo.
echo [OK] Classroom 3-9 installed.
echo - Auto-start on boot: YES
echo - Auto-restart Chrome: YES
echo - Remove: run remove_3-9.bat
timeout /t 3
