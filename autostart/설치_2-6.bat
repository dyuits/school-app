...
set "STARTUP_BAT=%STARTUP%\교실알림_2-6.bat"
> "%STARTUP_BAT%" echo @echo off
>> "%STARTUP_BAT%" echo start "" /min powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File "%INSTALL_DIR%\교실알림.ps1" -CLS "2-6"

:: 즉시 실행
start "" /min powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File "%INSTALL_DIR%\교실알림.ps1" -CLS "2-6"
