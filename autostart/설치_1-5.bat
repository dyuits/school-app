...
set "STARTUP_BAT=%STARTUP%\교실알림_1-5.bat"
> "%STARTUP_BAT%" echo @echo off
>> "%STARTUP_BAT%" echo start "" /min powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File "%INSTALL_DIR%\교실알림.ps1" -CLS "1-5"

:: 즉시 실행
start "" /min powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File "%INSTALL_DIR%\교실알림.ps1" -CLS "1-5"
