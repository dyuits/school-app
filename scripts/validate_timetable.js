...
assert(fs.readFileSync(path.join(root, 'docs/js/app.js'), 'utf8').includes('function ensureAfterSchoolAttendance()') && teacherPopupMarkup.includes('js/afterschool.js?v=20260831-3'), '방과후학교 출석부 런타임 재로딩 보호 누락');
...
assert(afterSchoolSource.includes('function teacherPhone(') && afterSchoolSource.includes('slice(-4)') && afterSchoolSource.includes('window.afterSchoolUnlockCurrent'), '연락처 전화번호 뒷 4자리 출석부 확인 누락');
