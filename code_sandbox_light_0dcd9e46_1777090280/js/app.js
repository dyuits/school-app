...
function parseCellValue(v, teacher) {
  ...
  // 시간강사/산학협력교사/체육순회/온라인 체크 (교사명으로 판별 — PDF 민트색 셀)
...
function renderSwapTable() {
  ...
  const searchVal = (qs('#swapSearch')?.value || '').trim().toLowerCase();
  const teachers = ALL_TEACHERS.filter(t => !searchVal || t.includes(searchVal));
    ...
    DAYS.forEach((day, di) => {
      PERIODS.forEach((period, pi) => {
        ...
        if (isLunchSlot && !val) {
          ...
          if (info.isOnline) {
            // 온라인 수업: 교사 없음 → 민트 처리, 클릭 불가
            cellStyle = 'style="background:#e8f5e9;border-color:#81c784;cursor:default;"';
            clickable = '';
            cellContent = `<div class="cell-inner">
              <span class="cell-subject" style="color:#2e7d32;font-size:9px;">온라인</span>
              <span class="cell-class" style="color:#388e3c;">${info.classLabel}</span>
            </div>`;
          } else if (info.isMint) {
            ...
            cellContent = `<div class="cell-inner">
              <span class="cell-subject">${info.subject}</span>
              <span class="cell-class ${info.isSelect ? 'cell-select-badge' : ''}">${info.classLabel}</span>
            </div>`;
          } else {
            cellContent = `<div class="cell-inner">
              <span class="cell-subject">${info.subject}</span>
        ...
        const cellTitle = cellInfo?.isOnline
          ? '온라인 수업 (교사 없음 — 교체·대체 불가)'
          : cellInfo?.isMint
            ? (teacher === '체육순회' ? '체육순회 수업 (교체·대체 불가)' : '시간강사·산학협력교사 수업 (교체·대체 불가)')
            : `${teacher} ${day}${period}교시`;
...
function findSwapCandidates(myTeacher, myDay, myPeriod, myVal) {
  const myInfo = parseCellValue(myVal, myTeacher);
  ...
  ALL_TEACHERS.forEach(other => {
    ...
    DAYS.forEach(d => {
      PERIODS.forEach(p => {
        ...
        if (!otherVal) return;
        const otherInfo = parseCellValue(otherVal, other);
...
function renderTeacherListPanel() {
  const searchVal = (qs('#teacherScheduleSearch')?.value || '').toLowerCase();
      ...
      <table class="teacher-detail-table" style="min-width:500px;">
        ...
        <tbody>
          ${PERIODS.map(p => {
            ...
            if (p === lunchAfter + 1) {
              lunchRow = `<tr>
                <td class="period-label" style="font-size:9.5px;color:#b8860b;font-style:italic;">점심</td>
                ${DAYS.map(()=>`<td class="lunch-break">${lunchLabel}</td>`).join('')}
              </tr>`;
            }
            const row = `<tr>
              ...
              ${DAYS.map(d => {
                ...
                if (!val) return `<td style="color:var(--txt-light);font-size:11px;">공강</td>`;
                const info = parseCellValue(val, teacher);
                let tdStyle = '';
                ...
                if (info.isOnline) {
                  ...
                  tdStyle = 'background:var(--cell-mint-bg);border:1px solid var(--cell-mint-bd);cursor:default;';
                  clickFn = ''; // 민트: 클릭 불가
                  const mintLabel = teacher === '체육순회' ? '[순회]' : '[강사]';
                  return `<td style="${tdStyle}" title="${teacher === '체육순회' ? '체육순회(교체불가)' : '시간강사·산학협력(교체불가)'}">
                    <div class="cell-inner">
                      <span style="font-size:11.5px;font-weight:700;">${info.subject}</span>
                      <span style="font-size:9px;color:var(--cell-mint-bd);">${mintLabel}${info.classLabel}</span>
                    </div>
                  </td>`;
                } else if (info.isSelect) {
...
function renderMeetingTab() {
  ...
  if (!listEl) return;
  const searchVal = (qs('#meetingSearch')?.value || '').toLowerCase();
  ...
  if (!filtered.length) {
    ...
    listEl.innerHTML = filtered.map(t => {
      ...
      return `<button class="side-btn-item meeting-teacher-btn ${selected ? 'selected' : ''}"
        ...
        <span class="meeting-btn-name">${t}</span>
...
function renderLabDetail(labName) {
  ...
  PERIODS.forEach(p => {
    ...
    DAYS.forEach(d => {
      ...
      if (cell) {
        html += `<td class="lab-cell-used" title="${cell}">${cell}</td>`;
      } else {
...
function verifyRosterPassword() {
  ...
  // 모든 선생님 중 일치하는 번호가 있어도 허용 (관리 편의)
  if (!matched) {
    matched = STAFF_CONTACTS.some(c => {
      if (!c.phone || c.phone === '-') return false;
      const digits = c.phone.replace(/-/g,'');
      return digits.slice(-4) === pw;
    });
...
function renderRosterBody(cls) {
  ...
  const { teacher, dept, students } = data;
  ...
  let html = `
    <div style="display:flex;gap:12px;margin-bottom:14px;flex-wrap:wrap;">
      ...
      <div class="roster-info-chip"><i class="fas fa-school"></i> 학과: <strong>${dept}</strong></div>
      <div class="roster-info-chip"><i class="fas fa-users"></i> 총 <strong>${students.length}</strong>명 (남 ${maleStudents.length} / 여 ${femaleStudents.length})</div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
  `;

  // 남학생 테이블
  html += buildRosterTable('남학생', maleStudents, '#dae3ff', '#2b4a7a');
  // 여학생 테이블
  html += buildRosterTable('여학생', femaleStudents, '#fde8ed', '#c0392b');
  ...
  html += `
    <div style="margin-top:16px;">
      <div style="font-size:12px;font-weight:700;color:var(--txt-mid);margin-bottom:8px;">📋 전체 명렬 (${students.length}명)</div>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:4px;">
        ${students.map(s=>`
          <div style="padding:5px 8px;background:var(--bg-soft);border-radius:6px;font-size:12px;display:flex;justify-content:space-between;">
            <span style="color:var(--txt-light);">${s.no}</span>
            <span style="font-weight:600;color:var(--txt-dark);">${s.name}</span>
            <span style="font-size:10px;color:${s.gender==='남'?'#2b4a7a':'#c0392b'};font-weight:600;">${s.gender}</span>
          </div>`).join('')}
      </div>
    </div>`;
...
function buildRosterTable(title, students, bg, color) {
  if (!students.length) return '';
  return `<div>
    <div style="font-size:12px;font-weight:700;margin-bottom:6px;color:${color};background:${bg};padding:5px 10px;border-radius:6px;">${title} (${students.length}명)</div>
    <table style="width:100%;border-collapse:collapse;font-size:12px;">
      <thead>
        <tr style="background:${bg};">
          <th style="padding:5px;text-align:center;border-bottom:1px solid var(--border-lt);color:${color};">번호</th>
          <th style="padding:5px;text-align:left;border-bottom:1px solid var(--border-lt);color:${color};">이름</th>
        </tr>
      </thead>
      <tbody>
        ${students.map((s,i)=>`
          <tr style="${i%2===0?'background:var(--bg-soft);':''}">
            <td style="padding:4px 5px;text-align:center;color:var(--txt-light);">${s.no}</td>
            <td style="padding:4px 5px;font-weight:600;color:var(--txt-dark);">${s.name}</td>
          </tr>`).join('')}
      </tbody>
    </table>
  </div>`;
...
function renderBlockTab() {
  ...
  if (!listEl) return;
  const searchVal = (qs('#blockSearch')?.value || '').toLowerCase();
