...
for %%i in ("%~dp0..\classroom\3-10.html") do set "HTML=%%~fi"
if not exist "%HTML%" (echo 파일을 찾을 수 없습니다. ^& pause ^& exit /b 1)
set "URL=%HTML:\=/%"
set "URL=file:///%URL%"

set "USERDATA=%LOCALAPPDATA%\교실알림\3-10"
...
>> "%VBS%" echo sURL = "%URL%"
>> "%VBS%" echo sUserData = oShell.ExpandEnvironmentStrings("%%LOCALAPPDATA%%") ^& "\교실알림\3-10"
...
>> "%VBS%" echo       If InStr(p.CommandLine, "교실알림\3-10") ^> 0 Then IsRunning = True
>> "%VBS%" echo     End If
>> "%VBS%" echo   Next
